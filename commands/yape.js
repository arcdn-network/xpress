...
function registerYapeCommand(bot) {
  bot.onText(/\/yape(.*)/, async (msg, match) => {
    ...
    if (!isDentroDeHorario()) {
      const msg = '🕙 *El servicio de vouchers está disponible de 8:00 a.m. a 10:00 p.m.*.';
      return bot.sendMessage(chatId, msg, replyOpts);
    }
    ...
    try {
      const { buffer } = await generateVoucher({ monto, nombre, digitos, mensaje, destino });
